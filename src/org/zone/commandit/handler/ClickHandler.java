...
public class ClickHandler {
	...
	public void copySign() {
		...
		LuaCode clone = plugin.getCodeBlocks().get(location).clone(
				player.getName());
	...
	public boolean onInteract(Action action) {
		...
		if (state != null) {
			...
			case BATCH_REDSTONE:
				...
				break;
			default:
				return new Executor(plugin, player, location, action)
						.run();
			}
			return true;
		} else {
	...
	public void removeSign(boolean batch) {
		...
		if (!batch) {
			if (plugin.getPlayerCode().containsKey(player)) {
				plugin.getPlayerStates().put(player, PlayerState.ENABLE);
				plugin.getMessenger().sendMessage(player,
						"information.code_in_clipboard");
			} else {
				plugin.getPlayerStates().remove(player);
			}
		}
