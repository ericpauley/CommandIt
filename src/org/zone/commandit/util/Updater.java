...
public class Updater {
	...
	public void init() {
		if (task == null && active) {
			task = plugin
					.getServer()
					.getScheduler()
					.runTaskTimerAsynchronously(plugin, new UpdateTask(), 0,
							20 * 60 * 15);
		}
	}
	...
	public Updater(CommandIt plugin, File pluginFile) {
		...
		if (currentVersion.build == 0) {
			plugin.getLogger().warning(
					"Running an in-house dev build! Auto-update disabled!");
			return;
		}
	...
	public void installUpdate(CommandSender cs, Version newVersion, String fetch) {
		...
		try {
			fl = new File(plugin.getServer().getUpdateFolderFile(),
					pluginFile.getName());
			dl = new URL(fetch);
			FileUtils.copyURLToFile(dl, fl);
			Message.sendMessage(cs, "update.finished",
					((double) System.nanoTime() / 1000000 - t) / 1000);
		} catch (Exception e) {
	...
	private class UpdateTask implements Runnable {
		...
		public void run() {
			...
			try {
				Configuration c = YamlConfiguration.loadConfiguration(source
						.openStream());
				Version available = Version.parse(c.getString("version"));
				if (available.getBuild() > currentVersion.getBuild()) {
					if (auto) {
						installUpdate(plugin.getServer().getConsoleSender(),
								available, "http://dev.bukkit.org/media/files/"
										+ c.getString("download"));
						availableVersion = available;
					} else {
						availableVersion = available;
						toFetch = "http://dev.bukkit.org/media/files/"
								+ c.getString("download");
					}
				}
			} catch (IOException e) {
